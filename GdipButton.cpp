...
BOOL CGdipButton::LoadStdImage(UINT id, LPCTSTR pType)
{
	...
	bool ret = m_pStdImage->Load(id, pType);
	m_bMultiLoad = true;
	return ret;
}
...
BOOL CGdipButton::LoadAltImage(UINT id, LPCTSTR pType)
{
	...
	bool ret = (m_pAltImage->Load(id, pType));
	m_bMultiLoad = TRUE;
	return ret;
}
...
HBRUSH CGdipButton::CtlColor(CDC* pScreenDC, UINT nCtlColor) 
{
	if (!m_bHaveBitmaps)
	{
		...
		if (m_dcStd.m_hDC == NULL)
		{
			...
			if(m_dcGS.m_hDC == NULL)
			{
				...
				float width = (float)m_pStdImage->m_pBitmap->GetWidth();
				float height = (float)m_pStdImage->m_pBitmap->GetHeight();
				m_std_bmp_width = width;
				m_std_bmp_height = height;
				RectF grect; grect.X=0, grect.Y=0; grect.Width = width; grect.Height = height;
		...
		if( (m_dcAlt.m_hDC == NULL) && m_bHaveAltImage )
		{
			...
			if(m_dcAltH.m_hDC == NULL)
			{
				...
				graphics.DrawImage(*m_pAltImage, grect, 0, 0, rect.Width(), rect.Height(), UnitPixel, &ia);
	...
	if (m_bMultiLoad)
	{
		...
		if (1)
		{
			...
			//bmp.CreateCompatibleBitmap(&clDC, rect.Width(), rect.Height());
			//pOldBitmap = m_dcBk.SelectObject(&bmp);
			////changed by zhiyuanc 20170510 start 图片和控件大小匹配
			//m_dcBk.BitBlt(0, 0, rect.Width(), rect.Height(), &clDC, rect1.left, rect1.top, SRCCOPY);
			////BITMAP get_bmp;
			////bmp.GetBitmap(&get_bmp);
			////m_dcBk.StretchBlt(0, 0, rect.Width(), rect.Height(), &clDC, 0, 0,get_bmp.bmWidth, get_bmp.bmHeight, SRCCOPY);
			////changed by zhiyuanc 20170510 end
			//bmp.DeleteObject();
		}
...
{
	GetParent()->InvalidateRect(m_rgn_rect);
	GetWindowRect(&m_rgn_rect);
	GetParent()->ScreenToClient(&m_rgn_rect);
	CRect rect;
	GetClientRect(rect);
	if (m_isRgn)
	{
		CRgn rgnTmp;
		rgnTmp.CreateRoundRectRgn(rect.left + m_rgn_x, rect.top + m_rgn_y, rect.right - m_rgn_x, rect.bottom - m_rgn_y, 2 * m_rgn_x, 2 * m_rgn_y);
		pDC->SelectClipRgn(&rgnTmp);
		GetParent()->InvalidateRgn(&rgnTmp);
		//SetWindowRgn(rgnTmp, TRUE);
	}
	//pDC->BitBlt(0, 0, rect.Width(), rect.Height(), &m_dcBk, 0, 0,SRCCOPY);
	//为了使透明照片在放大缩小后可以正常显示，使用以下绘制背景 fwl 20171011
	pDC->StretchBlt(0, 0, rect.Width(), rect.Height(), &m_dcBk, 0, 0,10,10, SRCCOPY);
}
...
void CGdipButton::PaintBtn(CDC *pDC)
{
	...
	dc.TextOut(m_show_text_x, m_show_text_y, m_str_text);
}
...
void CGdipButton::DrawItem(LPDRAWITEMSTRUCT lpDIS) 
{
	...
	if(m_bIsToggle && bIsPressed && !m_bStdDC)
	{
...
void CGdipButton::SetSelected(bool bSelected)
{
	;
}

//add by zhiyuanc 20170630
void CGdipButton::setRgn()
{
	CRgn rgnTmp;
	RECT rc;
	GetClientRect(&rc);
	rgnTmp.CreateRoundRectRgn(rc.left + m_rgn_x, rc.top + m_rgn_y, rc.right - m_rgn_x, rc.bottom - m_rgn_y, 2 * m_rgn_x, 2 * m_rgn_y);
	SetWindowRgn(rgnTmp, TRUE);
}

//add by zhiyuanc 20170704 交换基本图片和

void CGdipButton::SetStdDC()
{
	m_bStdDC = true;
	m_nCurType = STD_TYPE;
	Invalidate();
}
void CGdipButton::SetAltDC()
{
	m_bStdDC = false;
	m_nCurType = ALT_TYPE;
	Invalidate();
...
void CGdipButton::setText(CString str_text, int show_x, int show_y)
{
	m_str_text = str_text;
	m_show_text_x = show_x;
	m_show_text_y = show_y;
}